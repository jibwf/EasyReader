...
- PWA-aligned page structure (default landing on bookshelf):
	- `书架` / `搜索` / `阅读` / `设置`
- Paging-first reader baseline for novel content
...
- Server-first offline pipeline integration:
	...
	- Local chapter read path prefers client cache first, then server
...
## Next milestones
- Replace file cache with Room + structured offline queue
